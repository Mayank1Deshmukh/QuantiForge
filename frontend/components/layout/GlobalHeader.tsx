...
function DaemonStatusIndicator() {
  ...
  const color = { online: "#22c55e", offline: "#ef4444", training: "#f59e0b" }[status]
  const label = { online: "Daemon Online", offline: "Daemon Offline", training: "Training Active" }[status]
  return (
    <div className="flex items-center gap-2 text-sm">
      ...
      {deviceName && status !== "offline" && (
        <span className="text-[#404040] text-xs font-mono hidden lg:inline">{deviceName}</span>
      )}
    </div>
  )
}
...
function ActiveJobPill() {
  ...
  return (
    <div className="flex items-center gap-2 bg-[#171717] border border-[#404040] rounded-full px-3 py-1 text-sm">
      <span className="w-2 h-2 rounded-full bg-[#f59e0b] shrink-0" style={{ animation: "pulse 1.5s ease-in-out infinite" }} />
      <span className="font-mono text-[#d4d4d4] text-xs">
        Training · {runId?.slice(0, 8)} · {config?.backbone ?? "—"} ·{" "}
        {status === "dispatched" ? "Starting…" : `Epoch ${epoch}/${total}`}
      </span>
      <button onClick={handleCancel} className="ml-1 text-[#ef4444] hover:text-red-300 text-xs transition-colors">
        Cancel
      </button>
    </div>
  )
...
export function GlobalHeader() {
  ...
  return (
    <header className="h-[60px] flex items-center justify-between px-4 bg-[#0a0a0a] border-b border-[#404040] shrink-0 z-20">
      <span className="font-mono font-bold text-lg tracking-tight text-[#fafafa]">QuantiForge</span>
      <ActiveJobPill />
      <div className="flex items-center gap-4">
        <DaemonStatusIndicator />
        <button
          onClick={() => setSettingsOpen(true)}
          className="p-1.5 rounded hover:bg-[#262626] transition-colors text-[#a3a3a3] hover:text-[#fafafa]"
          title="Settings"
        >
          <Settings size={18} />
        </button>
