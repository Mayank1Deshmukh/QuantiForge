  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <input
        ...
        className="bg-[#262626] border border-[#404040] rounded px-3 py-1.5 text-sm font-mono text-[#d4d4d4] placeholder-[#404040] focus:outline-none focus:border-[#06b6d4]"
      />
    </div>
  )
}
  ...
  const testConnection = () => {
    ...
    try {
      ...
      const timeout = setTimeout(() => {
        ws.close()
        setPingResult("Timeout — no response")
      }, 5000)
      ws.onopen = () => {
      ...
      ws.onmessage = (e) => {
        ...
        try {
          ...
          if (msg.status === "READY") {
            setPingResult(`Connected — ${msg.device_name ?? "Unknown device"}`)
          }
        } catch {
          setPingResult("Got response (unreadable)")
        }
        ws.close()
      }
      ws.onerror = () => {
        clearTimeout(timeout)
        setPingResult("Connection refused")
      }
    } catch {
      setPingResult("Invalid URL")
    }
  }
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-[#262626] border border-[#404040] rounded px-3 py-1.5 text-sm font-mono text-[#d4d4d4] focus:outline-none focus:border-[#06b6d4]"
                  />
                  <button
                    onClick={testConnection}
                    className="px-3 py-1.5 bg-[#262626] hover:bg-[#404040] border border-[#404040] rounded text-sm text-[#a3a3a3] transition-colors"
                  >
                    Test
                  </button>
                </div>
            ...
            <section>
              ...
              <div className="flex flex-col gap-3">
                <div className="flex flex-col gap-1">
                  ...
                  <select
                    defaultValue={localStorage.getItem("qf_default_ticker") ?? "SPY"}
                    onChange={(e) => localStorage.setItem("qf_default_ticker", e.target.value)}
                    className="bg-[#262626] border border-[#404040] rounded px-3 py-1.5 text-sm text-[#d4d4d4] focus:outline-none focus:border-[#06b6d4]"
                  >
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex gap-2">
                    {([1, 10, 30, 60] as const).map((s) => (
                      <button
                        key={s}
                        onClick={() => localStorage.setItem("qf_sim_speed", String(s))}
